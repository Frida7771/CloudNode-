...
const sendEmail = async (token, email) => {
  try {
    ...
    logger.info(`SendGrid API key retrieved successfully`);

    const verificationLink = `https://${process.env.DOMAIN}/api/users/user/verify?token=${token}`;
    const msg = {
      to: email,
      from: "no-reply@yourdomain.com", // Your verified SendGrid sender email
      subject: "Verify Your Email - HealthWeb",
      text: `Please click the link below to verify your email: ${verificationLink}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2>Email Verification</h2>
          <p>Thank you for registering with HealthWeb!</p>
